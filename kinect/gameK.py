...
def gameK():
    ...
    drw=[pdb.gimp_layer_new(img,320,240,RGB,"kinect0",100.0,NORMAL_MODE),pdb.gimp_layer_new(img,320,240,RGB,"kinect1",0.0,NORMAL_MODE),pdb.gimp_layer_new(img,320,240,RGB,"kinect2",0.0,NORMAL_MODE)]
    img.add_layer(drw[0], 0)
    img.add_layer(drw[1], 0)
    img.add_layer(drw[2], 0)
    ...
    while not done:
        ...
        if e.type == pygame.QUIT:
            ...
            if draw_skeleton:
                ...
                pygame.display.update()
                ctrlPoints = [int(lastxy[0]), int(lastxy[1]),int(Hand[0]), int(Hand[1])]
                lastxy = Hand
                if Hand[0] > LHand[0]:
                    pdb.gimp_paintbrush(drw[ZLayer],0,len(ctrlPoints),ctrlPoints,0,0)
                    pdb.gimp_displays_flush()
        elif e.type == KEYDOWN:
            if e.key == K_ESCAPE:
                done = True
