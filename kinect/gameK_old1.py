...
def gameK():
    ...
    def draw_skeletons(skeletons):
        for index, data in enumerate(skeletons):
            ...
            #lastxy = 
            return (int(HandPos[0]),int(HandPos[1])),(int(LHandPos[0]),int(LHandPos[1]))
            #return lastxy
    ...
    img = pdb.gimp_image_new(400,400,RGB)
    ...
    while not done:
        ...
        if e.type == pygame.QUIT:
            ...
            if draw_skeleton:
                Hand,LHand=draw_skeletons(skeletons)
                pygame.display.update()
                ctrlPoints = [int(lastxy[0]), int(lastxy[1]),int(Hand[0]), int(Hand[1])]
                lastxy = Hand
                if Hand[0] > LHand[0]:
                    pdb.gimp_paintbrush(drw,0,len(ctrlPoints),ctrlPoints,0,0)
                    pdb.gimp_displays_flush()
        elif e.type == KEYDOWN:
            if e.key == K_ESCAPE:
                done = True
